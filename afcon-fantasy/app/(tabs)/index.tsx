...
export default function HomeScreen() {
  ...
  // FIXED: Load from user_teams first, then fall back to teams
  const loadTeamData = async () => {
    try {
      if (!currentUser) return;

      console.log('🔵 Loading team data for user:', currentUser.uid);

      // TRY user_teams FIRST (for transfers)
      const userTeamRef = doc(db, 'user_teams', currentUser.uid);
      const userTeamSnap = await getDoc(userTeamRef);
      
      let teamData;
      
      if (userTeamSnap.exists()) {
        console.log('✅ Loaded from user_teams');
        teamData = userTeamSnap.data();
      } else {
        console.log('⚠️ user_teams not found, trying teams collection...');
        teamData = await getTeam(currentUser.uid);
        console.log('✅ Loaded from teams');
      }
      ...
      if (teamData) {
        ...
        if (teamData.starting_xi && teamData.starting_xi.length > 0) {
          console.log('👥 Loading players:', teamData.starting_xi);
          ...
          console.log('⚠️ No starting_xi found');
        }
      } else {
